...
function render( data, page=1, perpage = 5 ){
  ...
  //列表部分
  document.getElementById('tbody').innerHTML = pageData.reduce(function(temp,item){
    return temp + `
    <tr>
      <th>${item.title}</th>
      <th class="date"><input name="id" type="checkbox" value="${item.id}"> ${item.addtime} <button onclick="delItem(${item.id},this)">删除</button></th>
    </tr>
    <tr>
      <td colspan="2"><pre>${item.content}</pre></td>
    </tr>
    `;
  ...
  $.post('feedback/add',{
    ...
    if(res.err===0){
      ...
      document.getElementById('tbody').innerHTML = `
        <tr>
          <th>${info.title}</th>
          <th class="date">${info.addtime} <button onclick="delItem(${info.id},this)">删除</button></th>
        </tr>
        <tr>
