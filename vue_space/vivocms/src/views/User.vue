  ...
  <div class="panel">
    ...
    <div class="heading">
      ...
      <el-breadcrumb separator="/">
        ...
        <el-breadcrumb-item><a href="#/app/user">会员管理</a></el-breadcrumb-item>
    ...
    <div class="search">
      <div class="options">
        ...
        <el-input v-model.trim="keyword" placeholder="请输入id/账号/手机" size="small" clearable />
      </div>
    ...
    <div class="list">
      <el-table ref="table" :data="searchData" @selection-change="handleSelectionChange">
        ...
        <el-table-column  label="手机"  prop="mobile" width="160" />
        <el-table-column  label="性别"  prop="geder"  :formatter="formatGender" />
...
import { uuid, formatstamp, searchCd } from '../utils/index'
export default {
  data () {
  ...
  computed: {
    searchData () {
      return this.tableData.filter(item => {
        const { userid, username, mobile } = item
        const cd1 = this.state !== '' ? (item.state === this.state) : true
        const cd2 = this.keyword !== '' ? searchCd(this.keyword, userid, username, mobile) : true
        return cd1 && cd2
      })
    }
  },
  methods: {
    ...
    handelChange (state, id) {
      this.$http.post('user/state', { state, id }).then(res => {
        if (res.result.err > 0) {
    ...
    delItem (id) {
      this.$confirm('您确定要删除此用户吗？', '提示').then(() => {
        this.$http.post('user/remove', { id }).then(res => {
          if (res.result.err > 0) {
            this.$message.error(res.result.msg)
          } else {
            this.$message.success(res.result.msg)
    ...
    batchDel () {
      this.$confirm('您确定要删除所选的用户吗？', '提示').then(() => {
        this.$http.post('user/remove', { id: this.ids }).then(res => {
          if (res.result.err > 0) {
            this.$message.error(res.result.msg)
          } else {
            this.$message.success(res.result.msg)
